import { Rating, RatingEdge, Teacher } from "../types/teacher";
...
export async function getProfessorDifficultyRatings(professorID: string, courseID: string) {
...
export async function getProfessorRatings(professorID: string, courseID: string) {
  ...
  const allRatings: Rating[] = professorRatings.data?.node?.ratings?.edges?.map((edge: RatingEdge) => edge.node as Rating);
  return allRatings;
}
